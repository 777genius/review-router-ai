...
describe("hosted Codex relay routes", () => {
  ...
  it("maps a missing admitted review request to a distinct grant status", async () => {
    const app = Fastify({ logger: false });
    await registerHostedCodexRelayRoutes(app, {
      enabled: true,
      grants: {
  ...
  it("maps custody unreadiness to a retryable grant status", async () => {
    const app = Fastify({ logger: false });
    await registerHostedCodexRelayRoutes(app, {
      enabled: true,
      grants: {
        issue: async () => {
          throw new Error("hosted_codex_custody_not_ready:reconcile_failed");
        },
      },
      commentTokens: { issue: vi.fn() },
      authorization: { authorize: vi.fn() },
