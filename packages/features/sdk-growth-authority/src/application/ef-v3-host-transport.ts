...
function response(
  value: { readonly wire: Uint8Array; readonly wireDigest: Digest },
  kind: "grant" | "receipt",
): Uint8Array {
  if (
    !(value.wire instanceof Uint8Array) ||
    value.wire.byteLength === 0 ||
    value.wire.byteLength > 32 * 1024 * 1024
  )
    throw new TypeError("ef-v3-wire-invalid");
  const wire = Uint8Array.from(value.wire);
  if (digest(wire) !== value.wireDigest)
    throw new TypeError("ef-v3-wire-digest-mismatch");
  const parsed = object(
    JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(wire)),
  );
  if (parsed.schemaVersion !== efV3Schema || parsed.kind !== kind)
    throw new TypeError("ef-v3-response-invalid");
  return wire;
}
