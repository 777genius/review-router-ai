...
import {
  ...
  SDK_GROWTH_CHECK_NAME,
} from "../application/check-identity-policy";

describe("SDK growth check reservation", () => {
  it("reserves the check name and status context across trim and case", () => {
    expect(isSdkGrowthReservedCheckIdentity(SDK_GROWTH_CHECK_NAME)).toBe(true);
    expect(
      isSdkGrowthReservedCheckIdentity(" reviewrouter / sdk growth authority "),
    ).toBe(true);
    expect(() => assertUnreservedCheckIdentity(SDK_GROWTH_CHECK_NAME)).toThrow(
      "sdk_growth_check_identity_reserved",
    );
  });
