...
describe("locked provider new-work eligibility", () => {
  ...
  it.each([
    ["acquisition", "compatibility", 0],
    ["acquisition", "compatibility", 1],
    ["replay", "compatibility", 0],
    ["replay", "compatibility", 1],
    ["replay", "provider", 0],
    ["replay", "lease", 0],
    ["replay", "provider", 1],
    ["replay", "lease", 1],
    ...
    "%s rechecks %s after identity wait at deadline offset %s",
    async (mode, deadlineSource, offset) => {
      const input = fixture();
      const start = input.at.getTime();
      const deadline = new Date(start + 1000);
      let databaseNow = start;
      const order: string[] = [];
      const namespace = {
      ...
      const lease = {
        ...
        githubRunAttempt: "1",
        status: "preleased",
        leaseKey: `${scope.providerInstanceId}:1:1:identity-v1`,
        mutationEpoch: 1n,
      ...
      const provider = {
        ...
        accountFingerprintSalt: "salt",
      };
      const tx = {
        $executeRawUnsafe: vi.fn(),
        $queryRaw: vi.fn(async (q: { strings: readonly string[] }) => {
          const sql = q.strings.join("");
          if (sql.includes("FOR UPDATE OF identity")) {
            const replay = sql.includes("identity, repository, installation");
            order.push(replay ? "replay lock" : "identity lock");
          ...
          if (sql.includes('FROM "CodexOAuthWorkflowCompatibility"'))
        ...
        codexOAuthLease: {
          ...
          upsert: vi.fn(async () => ({ id: "new-lease" })),
        },
      };
      ...
      const result = repository.acquirePrelease({
        repository: {
          ...
          identityBindingEpoch: "1",
        },
        providerInstanceId: scope.providerInstanceId,
        githubRunId: "1",
        githubRunAttempt: "1",
        verifiedWorkflowAttestation: input.verified,
        newWorkAdmissionBarrier: input.newWorkAdmissionBarrier,
      });
      if (offset >= 0) {
