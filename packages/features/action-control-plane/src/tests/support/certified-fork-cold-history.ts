import type { ForkExecutionInput } from "../../application/use-cases/execute-certified-fork-effect.js";
...
export async function coldHistoryFixture(
  ...
  let pending:
    ...
    | {
        operation: "execute";
        data: { expected: ForkComparison; command: ForkExecutionInput };
      }
    | null = null;
  ...
  async function append(
    ...
    function authorize(time: number) {
    ...
    if (
      ...
      const command: ForkExecutionInput =
        input.kind === "prepare"
          ? input
          : { kind: input.kind, effectKey: input.effectKey };
      pending = { operation: "execute", data: { expected, command } };
      const { executeCertifiedForkEffect } =
        await import("../../application/use-cases/execute-certified-fork-effect.js");
      const result = await executeCertifiedForkEffect(
        {
          ...deps,
          proofs: { ...proofs, authorize: ({ at }) => authorize(at)! },
        },
        { expected: prior, commandId, command },
      );
      c.requireFact(result.status === "committed");
      return;
    }
    pending = {
    ...
    await repository.compareAndCommit({
      ...
      expected,
      build(_current, time) {
        const authorityProof = authorize(time);
