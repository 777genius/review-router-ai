...
describe("current-main execute command restoration", () => {
  let executed: Awaited<ReturnType<typeof coldHistoryFixture>>;
  beforeAll(async () => {
    // Use the public writer so its command hash and event contract are covered.
    executed = await coldHistoryFixture({ execute: true });
  });

  it("cold-restores prepare, begin and retry written by executeCertifiedForkEffect", async () => {
    vi.resetModules();
    const { restoreCertifiedForkCheckpoint } =
      await import("../infrastructure/proofs/restore-certified-fork-checkpoint.js");
    const commands = Object.values(executed.disk.facts).filter(
      (f) => f.kind === "command" && f.payload.preimage.operation === "execute",
    );
    expect(
      new Set(
        commands.map(
          (f) =>
            f.kind === "command" &&
            (f.payload.preimage.data as { command: { kind: string } }).command
              .kind,
        ),
      ),
    ).toEqual(new Set(["prepare", "begin", "retry"]));
    const restored = await restoreCertifiedForkCheckpoint(
      controlledColdSource(copy(executed.disk)),
      executed.familyKey,
    );
    expect(restored.versions).toHaveLength(executed.disk.versions.length);
  ...
  it.each([
    ...
    "zero events",
    "multiple events",
    "wrong archive operation",
  ])("rejects execute %s even with recomputed hashes", async (variant) => {
    const disk = copy(executed.disk) as Disk;
    const fact = Object.values(disk.facts).find(
      (f) =>
        f.kind === "command" &&
        f.payload.preimage.operation === "execute" &&
        (f.payload.preimage.data as { command: { kind: string } }).command
          .kind === "begin",
    )!;
    if (fact.kind !== "command") throw new Error("missing execute fixture");
    const version = disk.versions.find(
      (v) => v.snapshot.version === fact.payload.version,
    )!;
    // End at the corrupted command: no later comparison can mask acceptance.
    disk.versions = disk.versions.slice(0, disk.versions.indexOf(version) + 1);
    const prior = disk.versions.at(-2)!;
    const p = fact.payload.preimage;
    const data = p.data as Record<string, unknown>;
    const command = data.command as Record<string, unknown>;
    if (variant === "unknown operation") p.operation = "execute-fabricated";
    if (variant === "reconcile kind") command.kind = "seal";
    if (variant === "extra field") data.extra = true;
    if (variant === "missing command") delete data.command;
    if (variant === "import shape") {
      data.inputs = [command];
      delete data.command;
    }
    if (variant === "wrong expected") data.expected = null;
    if (variant === "wrong input") command.effectKey = h(999);
    if (variant === "malformed input") {
      command.extra = true;
      version.snapshot.events.at(-1)!.input = copy(
        command,
      ) as (typeof version.snapshot.events)[number]["input"];
    }
    if (variant === "zero events")
      version.snapshot.events = copy(prior.snapshot.events);
    if (variant === "multiple events")
      version.snapshot.events.push(copy(version.snapshot.events.at(-1)!));
    if (variant === "wrong archive operation") version.operation = "renewClaim";
    const l =
      await import("../application/services/certified-fork-effect-ledger.js");
    const codec =
      await import("../infrastructure/prisma/certified-fork-archive-codec.js");
    const cp = version.snapshot.checkpoint!;
    const hash = l.commandHash(String(p.operation), data);
    version.receipt.commandHash = hash;
    fact.payload.commandHash = hash;
    cp.position!.commandHash = hash;
    fact.payload.authorityProofs = version.snapshot.events
      .slice(prior.snapshot.events.length)
      .flatMap((e) => (e.authorityProof === null ? [] : [e.authorityProof]));
    cp.anchorHash = l.checkpointAnchor(version.snapshot);
    cp.prefixLength = version.snapshot.events.length;
    cp.prefixHash = l.forkLedgerHash(version.snapshot);
    version.comparison = copy(
      codec.checkpointComparison(version.snapshot),
    ) as Mutable<typeof version.comparison>;
    vi.resetModules();
    const { restoreCertifiedForkCheckpoint } =
      await import("../infrastructure/proofs/restore-certified-fork-checkpoint.js");
    await expect(
