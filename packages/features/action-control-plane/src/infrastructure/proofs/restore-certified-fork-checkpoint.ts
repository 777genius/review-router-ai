...
function validateCommand(
  ...
  if (preimage.operation === "release" || preimage.operation === "renew") {
    ...
    // Current-main execution writes one exact command, unlike private imports.
    if (preimage.operation === "execute") {
      exactForkFields(preimage.data, ["expected", "command"]);
      requireFact(
        ["prepare", "begin", "retry"].includes(preimage.data.command.kind) &&
          delta.length === 1,
      );
      equal(delta[0]!.input, preimage.data.command);
    } else if (preimage.operation === "reconcile") {
      exactForkFields(preimage.data, ["expected", "command"]);
      requireFact(
