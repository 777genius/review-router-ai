...
describe("explicit checkout partition with an unchanged managed92 validator", () => {
  it("projects every admitted boundary through exactly106 to the same managed92 rows", () => {
    for (const source of [
      catalog,
      expanded,
      checkout96,
  ...
  it("pins current checkout106 to exact SQL107 bytes and keeps checkout105 valid", () => {
...
describe("complete filesystem checkout inventory", () => {
  ...
  it("reads checkout106 through original92 as identical frozen92 rows", async () => {
  ...
  it("admits only complete extension sets from actual filesystem bytes", async () => {
    ...
    const accepted = [
      0, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383,
    ];
  ...
  it("rejects changed or missing SQL and missing directories across the real inventory", async () => {
    ...
    for (const name of readdirSync(fixture.migrations).sort()) {
      ...
      if (name === migration107.migrationName)
