...
describe.skipIf(
  ...
  const previousTimestamp = new Date("2026-09-14T09:59:59.000Z");
  const metadataTimestamp = "2026-09-14T10:00:00Z";
    ...
    return new PrismaRepositoryWebhookHandler(db).handleGitHubRepositoryWebhook(
      {
        ...
        payload: {
          action: kind.startsWith("deleted")
            ? "deleted"
            : kind.startsWith("transferred")
              ? "transferred"
              : "edited",
          installation: {
  ...
  const after = (kind: Kind, target: Target) =>
    !kind.startsWith("metadata")
      ? { ...before(target), selected: false, lastSyncedAt: expect.any(Date) }
      : {
          lastSyncedAt: new Date(metadataTimestamp),
          selected: true,
          owner: "new",
          name: `renamed-${target.repositoryId}`,
  ...
  const result = (kind: Kind, target: Target) => ({
    ok: true,
    value: {
      processed: true,
      repository: kind.startsWith("metadata")
        ? `new/renamed-${target.repositoryId}`
        : `old/${target.repositoryId}`,
      status: kind.startsWith("metadata")
        ? "synced"
        : kind.startsWith("deleted")
          ? "unselected"
          : "reconnect_reselection_required",
    },
  });
    ...
    async (kind) => {
      ...
      const unrelatedBefore =
        await observer.repositoryConnection.findUniqueOrThrow({
          where: { id: unrelated.repositoryId },
        });
      const entered = latch();
      const release = latch();
      ...
      try {
        ...
        expect(await writing).toEqual(result(kind, target));
        expect(
          await observer.repositoryConnection.findUniqueOrThrow({
            where: { id: unrelated.repositoryId },
          }),
  ...
  it.each(["workspace", "installation", "both"] as const)(
    "discovers committed %s rebind while a webhook waits for the global guard",
    async (binding) => {
      const target = await fixture();
      ...
      try {
        await Promise.race([
          entered.promise,
          rebinding.then((value) => {
            if (!value.ok) throw value.error;
          }),
        ]);
        // Deliver the old installation's event. A changed installation must fence
        // the newly discovered binding; a workspace-only move can still sync.
        // Serializable conflicts may retry the whole guarded production callback.
        writing = outcome(mutate(writer, "metadata-repository", target));
        await waitBlocked("r128-writer");
        expect(await read(observer, target)).toEqual(before(target));
        release.release();
        expect(await rebinding).toEqual({ ok: true, value: undefined });
        const expectedKind =
          binding === "workspace"
            ? "metadata-repository"
            : "transferred-repository";
